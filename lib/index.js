...
const defaultTransformObject = obj => {
    ...
    if (isFunction(obj.toJSON)) { // The resolver has a "toJSON" method, invoke that
        obj                 = obj.toJSON();
 ...
 */
const createQrry = (mainResolver, {transformObject = null} = {}) => {
    ...
    const resolve = (resolver, query, context) => {
        ...
        if (isFunction(resolver)) {
            ...
            const key  = take(components, depth).join('.');
            ...
            return Promise.resolve(resolver(context, subQuery))
                ...
                .then(returnedResolver => resolve(returnedResolver, subQueryFactory({path: rest.join('.')}), context));
        }
        ...
        if (!path) { // We're at the end of the path chain
            ...
            if (isObject(resolver) && !resolver.__noResolve && !isArray(resolver)) {
                ...
                return props(mapValues(resolver, subResolver => {
                    ...
                    return resolve(subResolver, subQueryFactory({path: '', implicit: true}), context);
                }));
            }
            ...
            if (isObject(resolver)) {
                ...
                if (has(resolver, key))
                    return Promise.resolve(resolve(get(resolver, key), subQueryFactory({path: restPath}), context));
            }
     ...
     * @return {Promise}
     */
    const querySingle = (path, context) => {
        validatePath(path);
        const query = {path: convertPath(path)};
        return resolve(mainResolver, query, context);
    };

    const queryMultiple = multi(querySingle);

    queryMultiple.single  = querySingle;
    queryMultiple.resolve = resolve.bind(this, mainResolver);

    return queryMultiple;
};

module.exports = createQrry;