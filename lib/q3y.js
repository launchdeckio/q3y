...
/**
 * Create a function that, given the resolver object or function,
 * resolves the value at the given path or paths
 */
const q3y = (mainResolver, {
    ...
    const $ = (resolver, query, context) => {
        ...
        if (isFunction(resolver)) {
            ...
            return Promise.resolve(resolver(context, subQuery))
                ...
                .then(returnedResolver => $(returnedResolver, subQueryFactory({path: rest.join(separator)}), context));
        }
        ...
        if (!path) { // We're at the end of the path chain
            ...
            if (isObject(resolver)) {
                ...
                let restPath = tail(split(path, separator)).join(separator);

                let key = head(split(path, separator));
    ...
    const $single = (path, context) => {
        validatePath(path);
        const query = {path: convertPath(path, separator)};
